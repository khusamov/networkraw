...
folder are mapped to the application's root namespace, 'NetworkRaw'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "NetworkRaw"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'NetworkRaw' in the same way
as `"NetworkRaw/sass/src"`.
